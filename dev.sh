...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=destiny_project.example.com
...
function dkbuild {
    CD=$(pwd)
    cd $PROJ_BASE
    docker build -t destiny_project .
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development destiny_project npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop destiny_project
    docker rm destiny_project
    docker run --name destiny_project -d --env-file /home/ubuntu/destiny_project.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/destiny_project:/dkdata \
        destiny_project start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./destiny_project/
...
function dk {
    docker exec -it destiny_project $@
}
...
echo_green "Welcome to destiny_project's dev env"
